...
instance Show ForwardProp where
    show (ForwardProp fF iF aF oF x h label output state parameters inputStack) 
        = "ForwardProp" ++ " f: " ++ show fF ++ " i: " ++  show iF ++  " a: " ++ 
            ...
            show state  ++ "\n\n"
-- ++ " parameters: " ++ show parameters ++ show inputStack
...
coalg_cell (Fx (Cell state deltas innerCell), forwardProps, backProp)
  = let ForwardProp f i o a x h label output updatedState hyperparameters _ = head forwardProps
        prevState = if null forwardProps then [0] else stateF (head (tail (forwardProps))) -- 
    --   ForwardProp {state = prevState}                                     = head (tail (forwardProps ++ []))
        (weights_w, weights_u, biases) = hyperparameters
      
        BackProp dState_next deltaOut_next 
                    f_next 
        ...
        deltaB     = deltaGates
        backProp'  = BackProp dState dOut f nextLayerDeltas nextLayerWeightsW

      in  Cell updatedState (Deltas deltaW deltaU deltaB) (innerCell, tail forwardProps, backProp' )
...
alg2_cell (Cell state deltas (innerCell, delta_total)) 
    =   
        let Deltas {deltaW = deltaW1, deltaU = deltaU1, deltaB = deltaB1} = deltas
            Deltas {deltaW = deltaW2, deltaU = deltaU2, deltaB = deltaB2} = delta_total
...
-- [fB', iB', aB', oB'] trace (show (weights_w,deltaW_total))
--[fU', iU', aU', oU']
--[fW', iW', aW', oW']
